...
class EvalResult:
    ...
    def overall_score(self) -> float:
        ...
        Returns the mean of the three metric scores.
        """
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            "Evaluate the response using each rubric criterion. Return only a JSON "
        ...
        try:
            ...
            for criterion in rubric:
                value = float(candidate_scores[criterion])
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        # With paired/order-randomized evaluations, alternating entries represent
        # the two positions. Flag a consistent first-position advantage.
        positional_bias = False
        if len(score_groups) >= 2:
            group_averages = [sum(group) / len(group) for group in score_groups]
            first_position = group_averages[::2]
            second_position = group_averages[1::2]
            pairs = list(zip(first_position, second_position))
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        Computes each average, compares it with baseline, and reports regressions.
        """
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            ...
            values = [f"F{index:03d}", failure_type, root_cause, suggestion, "Open"]
    ...
    def generate_improvement_suggestions(
        ...
        suggestions_by_type = {
            ...
            "irrelevant": "Add intent-focused prompt examples so answers directly address the question",
            "incomplete": "Improve retrieval coverage and require answers to include every key condition",
            "off_topic": "Improve intent routing and add an explicit domain-scope instruction",
            "refusal": "Calibrate safety guardrails with valid in-scope examples to reduce false refusals",
            "unknown": "Review low-scoring cases manually and add representative regression tests",
        }
        ordered_types = sorted(categories, key=lambda name: (-categories[name], name))
        suggestions = [suggestions_by_type.get(name, suggestions_by_type["unknown"]) for name in ordered_types]

        general_suggestions = [
            "Add failed cases to the golden dataset and run them as CI regression tests",
            "Review retrieval traces and tune chunking, ranking, and generation prompts together",
            "Calibrate evaluation thresholds against a human-labeled sample before deployment",
        ]
